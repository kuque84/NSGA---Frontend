...
const AlumnosCursos = ({ alumno }) => {
  ...
  const fetchAlumnoCursoData = async () => {
    ...
    try {
      const alumnoCursoData = await fetchAlumnoCurso(id_alumno);
      alumnoCursoData[0] &&
        (() => {
  ...
  const handleSubmit = (event) => {
    ...
    if (id_inscripcion_curso) {
      //console.log('PUT');
      axios
        .put(`${API_URL}/inscripcioncurso/actualizar/${id_inscripcion_curso}`, alumnoCursoData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        })
        .then((response) => {
          Swal.fire('¡Actualizado!', 'Los datos del curso han sido actualizados.', 'success');
          handleEdit();
        })
        .catch((error) => {
          if (error.response && error.response.status === 409) {
            Swal.fire('Error', 'El alumno ya está inscripto en el curso seleccionado', 'error');
          } else {
            Swal.fire('Error', 'Hubo un error al actualizar los datos del curso', 'error');
          }
        });
    } else {
      //console.log('POST');
      axios
        ...
        .catch((error) => {
          if (error.response && error.response.status === 409) {
            ...
            Swal.fire('Error', 'Hubo un error al inscribir al alumno en el curso', 'error');
          }
        });
    }
  };
  ...
  const handleDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${API_URL}/inscripcioncurso/eliminar/${id_inscripcion_curso}`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
          })
          .then((response) => {
            Swal.fire('¡Eliminado!', 'El registro de inscripción ha sido eliminado.', 'success');
            //navigate('/alumnos');
            setId_ciclo('');
            setId_plan('');
            setId_curso('');
        ...
        <div className='relative mt-4 mb-6'>
          ...
          <label
            ...
            className={`peer-focus:font-medium absolute text-sm text-primary duration-300 transform ${
              cicloLectivo ? '-translate-y-6 scale-75' : '-translate-y-1 scale-100'
